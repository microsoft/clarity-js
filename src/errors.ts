...
class ErrorMonitor implements IComponent {
    ...
    private regexEscape = (str) => str.replace(/([.?*+^$&[\]\\(){}|<>-])/g, "\\$1");
    ...
    private logError(errorToLog) {
        ...
        // beyond an error threshold, continuing to notify the server of new errors loses meaning.
        // Pages with many errors tend to be stuck in error loops that originate from an earlier error
        if (this.errorCount <= 16) {
            ...
            let hrefRegex = new RegExp(this.regexEscape(location.href), "g");
            ...
            if (stack) {
                ...
                for (let potentialCompressable in fileNameCounts) {
                    ...
                    if (fileNameCounts[potentialCompressable] > 1) {
                        let escapedFileName = this.regexEscape(potentialCompressable);
