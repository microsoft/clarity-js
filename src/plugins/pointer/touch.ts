  ...
  if (touches) {
    for (let i = 0; i < touches.length; i++) {
      ...
      states.push({
        ...
        event: evt.type,
        pointer: "touch",
        x: "clientX" in touch ? Math.round(touch.clientX + de.scrollLeft) : null,
        y: "clientY" in touch ? Math.round(touch.clientY + de.scrollTop) : null,
        width: "radiusX" in touch ? Math.round(touch["radiusX"]) : ("webkitRadiusX" in touch ? Math.round(touch["webkitRadiusX"]) : 0),
        height: "radiusY" in touch ? Math.round(touch["radiusY"]) : ("webkitRadiusY" in touch ? Math.round(touch["webkitRadiusY"]) : 0),
